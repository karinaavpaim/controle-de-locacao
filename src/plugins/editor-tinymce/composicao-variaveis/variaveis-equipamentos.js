  ...
  return [
    {
      ...
      filhas: [
        {
          nome: "Tabela padrão",
          atributo: "Tabela_De_Equipamentos_Padrao",
          titulo: "Tabela de equipamentos padrão",
          inline: false,
          conteudo: function () {

            let equipamentos = orcamento.itens.filter(i => i.categoria == CATEGORIAS_ITEM.EQUIPAMENTO);

            var inicioTabela = `
            <table id="tabela-equipamentos-orcamento-locacao" style="width: 100%;" border="1"  >
              <thead>
            ...
            if (equipamentos.length < 1) {
              meioTabela = `<tr><td colspan="6"><center>Não há equipamentos.</center></td></tr>`;
            } else {
              equipamentos.map(e => {
                meioTabela += `                        
              <tr>
                ...
                <td style="text-align: right;">${mascaraDinheiro.aplicarMascaraParaRealComPrefixo(e.calcularValorTotalComoEquipamento())}</td>
              </tr>`;
              });
            }
            ...
            var fimTabela = ` 
              ...
              <p style="text-align: right;margin-top: -10px;" data-mce-style="text-align: right;">
                <span style="font-size: 14pt;" data-mce-style="font-size: 14pt;">
                  <strong>&nbsp;${mascaraDinheiro.aplicarMascaraParaRealComPrefixo(orcamento.calcularERetornarValoresItens().valorTotalEquipamentos)}</strong>
                </span>
              </p>`;
        ...
        {
          nome: "Tabela simples",
          atributo: "Tabela_De_Equipamentos_Simples",
          titulo: "Tabela de equipamentos simples",
          inline: false,
          conteudo: function () {

            let equipamentos = orcamento.itens.filter(i => i.categoria == 'EQUIPAMENTO');
            ...
            var meioTabela = ``;
            if (equipamentos.length < 1) {
              meioTabela = `<tr><td colspan="3"><center>Não há equipamentos.</center></td></tr>`;
            } else {
              equipamentos.map(e => {
                meioTabela += `                        
              <tr>
