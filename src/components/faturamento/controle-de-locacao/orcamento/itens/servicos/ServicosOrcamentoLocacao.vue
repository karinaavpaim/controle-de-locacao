      ...
      <v-flex sm12 lg12>
        ...
        <v-flex mt-2 xs12 v-show="(itens.filter(i => i.categoria === categoriaComponente).length == 0)">
          <v-layout wrap class="card-no-data py-12">
            <v-flex xs12>
              <span class="subheader">Não há serviços adicionados neste orçamento.</span>
            </v-flex>
          </v-layout>
        </v-flex>
      </v-flex>
            ...
            <v-row no-gutters>
              <v-col cols="6">
                <v-card-title class="display-block">
                  <h2 class="titulo-modal">{{ modoEdicao ? "Editar" : "Adicionar" }} serviço</h2>
                </v-card-title>
              </v-col>
              ...
              <v-layout wrap>
                ...
                <v-flex xs12 sm12 mx-2>
                  <v-row>
                    ...
                    <v-flex xs12 sm3 px-2>
                      <date-picker
                        :id="'datepicker-previsao-inicial-'+id"
                    ...
                    <v-flex xs12 sm3 px-2>
                      <date-picker
                        :id="'datepicker-previsao-final-'+id"
...
export default {
  mixins: [itensMixin],
  props: {
    id: { type: String, required: false, default: ()=> 'servicos-orcamento' },
  },
  components: {
    CampoDinheiro,
    DatePicker,
  ...
  data: () => ({
    categoriaComponente: CATEGORIAS_ITENS_ORCAMENTO_LOCACAO.SERVICO.valor,
    tiposDeProduto: TIPOS_PRODUTO_SERVICO,
    colunasServicos: COLUNAS_SERVICOS
  }),
  ...
  methods: {
    valoresDoItemParaFiltro(item){
      return [ 
        ...
        this.$options.filters.dinheiro(item.dataInicialLocacao),
