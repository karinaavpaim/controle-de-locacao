      ...
      <v-flex sm12 lg12>
              ...
              <tbody>
                <tr v-for="item in items" :key="item.identificador">
                  ...
                  <td class="text-right nowrap px-0 pr-1">
                    <v-tooltip bottom v-if="!item.movimentado">
                      <template v-slot:activator="{ on }">
                        <v-btn class="mx-1" x-small icon color="primary" @click="abrirModalParaEditarItem(item)" :id="'btn-editar-'+id" v-on="on" :disabled="item.movimentado">
                          <v-icon>edit</v-icon>
                        </v-btn>
                      </template>
                      <span>Editar</span>
                    ...
                    <v-tooltip v-if="item.movimentado" bottom>
                      <template v-slot:activator="{ on }">
                        <v-btn class="mx-1" x-small icon color="primary" @click="visualizarItem(item)" :id="'btn-visualizar-'+id" v-on="on">
        ...
        <v-flex mt-2 xs12 v-show="(itens.filter(i => i.categoria === categoriaComponente).length == 0)">
          <v-layout wrap class="card-no-data py-12">
            <v-flex xs12>
              <span class="subheader">Não há materiais adicionados neste orçamento.</span>
            </v-flex>
          </v-layout>
        </v-flex>
      </v-flex>
    ...
    <v-layout wrap>
      <v-dialog @keydown.esc="dialogItem = false" v-model="dialogItem" persistent max-width="65%">
        <v-card>
            ...
            <v-row no-gutters>
              <v-col cols="6">
                <v-card-title class="display-block">
                  <h2 class="titulo-modal" v-if="!item.movimentado">{{ modoEdicao ? "Editar" : "Adicionar" }} material</h2>
                  <h2 class="titulo-modal" v-else>Visualizar material</h2>
                </v-card-title>
              </v-col>

              <v-col cols="6">
                <v-alert text dense color="red" border="left" class="notificacao-alerta mt-3 mb-0 mr-4" v-if="mensagemDeErroValorTotalDoItem">
                  ...
                  <v-row>
                    <v-flex xs12 sm3 px-2>
                      <pesquisar-produto
                        ...
                        :identificadoresSetores="[configuracaoControleLocacao.secaoSetor.setorEstoquePrincipal, configuracaoControleLocacao.secaoSetor.setorEstoqueExpedicao]">
                        <template v-slot:selection="{produto}">{{produto.codigo}}</template>
                      </pesquisar-produto>
                    </v-flex>
                    ...
                    <v-flex xs12 sm9 px-2>
                      <v-text-field
                        ...
                        <template v-slot:append-outer>
                          <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                              <v-icon class="bg-icon" color="primary" :id="'btn-editar-descricao-'+id" v-text="'edit'" v-on="on" @click="editarDescricaoDoItem" :disabled="item.movimentado || !habilitaEditarDescricaoDoItem || produtoPadraoSelecionado()"></v-icon>
                            </template>
                            <span>Editar</span>
                          </v-tooltip>
                        </template>
                  ...
                  <v-row>
                    ...
                    <v-flex xs12 sm9 px-2>
                      <v-row>
                        ...
                        <v-flex xs3 sm4 px-2>
                          <date-picker
                            :id="'datepicker-saida-prevista-'+id"
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn v-show="!item.movimentado"  class="btn-primary" text @click="adicionarItemNaTabela" :id="'btn-salvar-'+id">
              Salvar
            </v-btn>
...
export default {
  mixins: [itensMixin],
  props: {
    id: { type: String, required: false, default: ()=> 'materiais-orcamento' },
  },
  components: {
    DatePicker,
    CampoDinheiro,
  ...
  data: () => ({
    categoriaComponente: CATEGORIAS_ITENS_ORCAMENTO_LOCACAO.MATERIAL.valor,
    tiposDeProduto: TIPOS_PRODUTO_MATERIAIS,
    colunasMateriais: COLUNAS_MATERIAIS,
  }),
